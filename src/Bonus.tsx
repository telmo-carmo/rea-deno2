...
interface EnhancedTableProps {
  numSelected: number;
  onRequestSort: (event: React.MouseEvent<unknown>, property: keyof BnData) => void;
...
function EnhancedTableHead(props:EnhancedTableProps) {
  ...
  const createSortHandler =
    (property: keyof BnData) => (event: React.MouseEvent<unknown>) => {
      ...
      <TableRow>
        <TableCell padding="checkbox">
          <Checkbox
            ...
            inputProps={{
              'aria-label': 'seleciona todos bonus',
            }}
          />
        </TableCell>
...
//   data: Data[];   // // res json is { "data": [ ...]}
// }

export default function Bonus() {
  const [order, setOrder] = React.useState<Order>('asc');
  const [orderBy, setOrderBy] = React.useState<keyof BnData>('ename');
  const [selected, setSelected] = React.useState<readonly string[]>([]);
  const [page, setPage] = React.useState(0);
  const [rowsPerPage, setRowsPerPage] = React.useState(5);
  const [loading, setLoading] = React.useState(true);
  const [error, setError] = React.useState(false);
  const [rows, setRows] = React.useState<BnData[]>([]);
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelecteds = rows.map((n) => n.ename);
